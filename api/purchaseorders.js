...
export default async function handler(req, res) {  // vercel servel less function starts here
  ...
  if (url.searchParams.has("securityCode")) {
    ...
    try {
      ...
      return res.status(200).send("✅ Authorization successful. You can now call /api/purchaseorders to fetch your details.");
    } catch (err) {
  ...
  if (!tokens.access_token) {
    return res.status(401).send("❌ Not authorized. Please visit /api/purchaseorders?auth to start.");
  }
  ...
  try {
    ...
    const apiUrl = `https://www.zohoapis.in/books/v3/purchaseorders?organization_id=${orgId}&page=${page}&per_page=${perPage}`;
    ...
    return res.status(500).send("Failed to fetch purchaseorders");
  }
}
