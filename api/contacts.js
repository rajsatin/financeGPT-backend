...
export default async function handler(req, res) {  // vercel servel less function starts here
  ...
  const url = new URL(req.url, baseURL);         //api/contacts
  const redirectPath = `${baseURL}${url.pathname}?securityCode`; // dynamic redirect URL
  ...
  if (url.searchParams.has("auth")) {
    const authUrl = `${process.env.ZOHO_API_DOMAIN}/oauth/v2/auth` +
      ...
      `&response_type=code` +
      `&access_type=offline` +
      `&redirect_uri=${redirectPath}`; // secuirtyCode
